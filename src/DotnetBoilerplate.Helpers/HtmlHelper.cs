...
namespace DotnetBoilerplate.Helpers
{
    public static class HtmlHelper
    {
        ...
        public static bool IsEmptyHtml(string text)
        {
            if (string.IsNullOrEmpty(text))
                return true;
            ...
            string pattern = "<[^<>^]+?>";
            string replacement = "";
            string result1 = Regex.Replace(text, pattern, replacement);
        ...
        public static string FormatText(string text, bool stripTags, bool convertPlainTextToHtml, bool allowHtml, bool resolveLinks)
        {
            if (string.IsNullOrEmpty(text))
                return string.Empty;
            ...
            try
            {
                ...
                if (resolveLinks)
                {
                    text = UrlHelper.FormatText(text);
                }
            }
            catch (Exception exc)
            {
